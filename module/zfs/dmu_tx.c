...
dmu_tx_delay(dmu_tx_t *tx, uint64_t dirty, hrtime_t last_smooth)
{
	...
	if (dirty > delay_min_bytes) {
		...
		min_tx_time = zfs_smoothing_scale *
		    dirty / (delay_min_bytes - dirty);
	}
	min_tx_time = MIN(min_tx_time, zfs_delay_max_ns);
	if (zfs_write_smoothing == 0 && now > tx->tx_start + min_tx_time)
		return;
	...
	if (dirty > delay_min_bytes) {
		dp->dp_last_smooth = now + zfs_write_smoothing * NANOSEC;
	}
